...
<div class="max-w-3xl mx-auto p-4">
    ...
    <form action="{{ route('products.store') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="category" id="category" value="{{ old('category') }}" placeholder="e.g. Food, Drinks" 
                   class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring focus:ring-blue-300">
        </div>

        <div>
            <label class="block mb-1 font-medium" for="price">Price</label>
            <input type="number" step="0.01" name="price" id="price" value="{{ old('price') }}" placeholder="Price in $" 
                   class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring focus:ring-blue-300">
        </div>

        <div>
            <label class="block mb-1 font-medium" for="quantity">Quantity</label>
            <input type="number" name="quantity" id="quantity" value="{{ old('quantity') }}" placeholder="Quantity" 
                   class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring focus:ring-blue-300">
        </div>
        ...
        <div>
            ...
            <textarea name="description" id="description" placeholder="Optional description" rows="4"
                      class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring focus:ring-blue-300">{{ old('description') }}</textarea>
        </div>

        <div class="flex gap-2">
            <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">Add Product</button>
